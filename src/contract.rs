...
use cosmwasm_std::{
    ensure, from_json, Addr, Binary, Response, StdError, StdResult, Storage, SubMsg, Uint128,
    WasmMsg,
};
...
// Struct to represent the Fractional NFT contract
pub struct FractionalNftContract {
    pub(crate) collection_address: Item<'static, Addr>,// Address of the NFT collection
    pub(crate) cw20_address: Item<'static, Option<Addr>>,// Optional address of the CW20 token contract
    pub(crate) vault: IndexedMap<'static, &'static str, VaultItem, VaultIndexes<'static>>,// Vault to store deposited NFTs
}
// Configuration struct for the CW20 token
#[cw_serde]
pub struct Cw20Config {
    pub code_id: u64,
    pub name: String,
    pub symbol: String,
    pub marketing: Option<InstantiateMarketingInfo>,// Optional marketing information
}
...
impl FractionalNftContract {
    ...
    // Ensure the CW20 token address is set
    pub fn ensure_cw20_token_set(&self, storage: &dyn Storage) -> Result<Addr, ContractError> {
    ...
    // Ensure the CW20 token address is not set
    pub fn ensure_no_cw20_token_set(&self, storage: &dyn Storage) -> Result<(), ContractError> {
    ...
    pub fn set_token_address(
        ...
        self.ensure_no_cw20_token_set(context.deps.storage)?;
    ...
    pub fn receive_nft(
        ...
        let cw20_address = self.ensure_cw20_token_set(context.deps.storage)?;
        let collection_address = self.collection_address.load(context.deps.storage)?;
    ...
    pub fn receive(
        ...
        let unwrapped_msg: ReceiveMsg =
            from_json(&msg).map_err(|error| ContractError::Std(error))?;
